...
port_alterid_set() {
    if [[ "on" != "$old_config_status" ]]; then
        read -rp "Please enter the connection port（default:1443）:" port
        [[ -z ${port} ]] && port="1443"
        read -rp "Please enter alterID (default: 64 only allows numbers):" alterID
...
v2ray_install() {
    ...
    mkdir -p /root/v2ray
    cd /root/v2ray || exit
    wget -N --no-check-certificate https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh
...
domain_check() {
    read -rp "Please enter your domain name information(eg:www.queenssh.com):" domain
    ...
    if [[ $(echo "${local_ip}" | tr '.' '+' | bc) -eq $(echo "${domain_ip}" | tr '.' '+' | bc) ]]; then
        echo -e "${OK} ${GreenBG} Domain name dns resolve IP Match with local IP ${Font}"
        sleep 2
    else
        echo -e "${Error} ${RedBG} Please make sure the domain name is added correctly A Record, otherwise it will not work normally V2ray ${Font}"
        echo -e "${Error} ${RedBG} Domain name dns resolve IP Does not match the local IP. Continue installation？（y/n）${Font}" && read -r install
        case $install in
        [yY][eE][sS] | [yY])
...
port_exist_check() {
    if [[ 0 -eq $(lsof -i:"$1" | grep -i -c "listen") ]]; then
        echo -e "${OK} ${GreenBG} $1 Port is not supported ${Font}"
        sleep 1
    else
        echo -e "${Error} ${RedBG} detected $1 The port is supported, the following is $1 Port occupation information ${Font}"
        lsof -i:"$1"
        echo -e "${OK} ${GreenBG} 5s Will try automatically kill process ${Font}"
        sleep 5
...
v2ray_conf_add_tls() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/tls/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
v2ray_conf_add_h2() {
    cd /etc/v2ray || exit
    wget --no-check-certificate https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/http2/config.json -O config.json
    modify_path
    modify_alterid
    modify_inbound_port
    modify_UUID
...
old_config_exist_check() {
    if [[ -f $v2ray_qr_config_file ]]; then
        echo -e "${OK} ${GreenBG} Old configuration file detected, whether to read the old file configuration [Y/N]? ${Font}"
        read -r ssl_delete
        case $ssl_delete in
        [yY][eE][sS] | [yY])
...
enable_process_systemd() {
    ...
    judge "Set up v2ray Self-start"
    if [[ "$shell_mode" != "h2" ]]; then
        systemctl enable nginx
        judge "Set up Nginx Self-start"
    fi
...
#}
acme_cron_update() {
    wget -N -P /usr/bin --no-check-certificate "https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/main/ssl_update.sh"
...
basic_information() {
    {
        ...
        echo -e "${Red} The underlying transmission security:${Font} tls "
    } >"${v2ray_info_file}"
}
show_information() {
    cat "${v2ray_info_file}"
...
tls_type() {
    if [[ -f "/etc/nginx/sbin/nginx" ]] && [[ -f "$nginx_conf" ]] && [[ "$shell_mode" == "ws" ]]; then
        ...
        echo "Please note,If you use Quantaumlt X / router / Old version Shadowrocket / Below 4.18.1 Version of V2ray core please choose Compatibility mode"
...
update_sh() {
    ol_version=$(curl -L -s https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh | grep "shell_version=" | head -1 | awk -F '=|"' '{print $3}')
    ...
    if [[ "$shell_version" < "$(sort -rV $version_cmp | head -1)" ]]; then
        ...
        [yY][eE][sS] | [yY])
            wget -N --no-check-certificate https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/install.sh
...
menu() {
    ...
    echo -e "\t---Modif by Gugun09---"
    echo -e "\thttps://github.com/Gugun09\n"
    ...
    echo -e "${Green}11.${Font} Install 4-in-1 BBR quick install script"
    ...
    2)
        ...
        ;;
    3)
        bash <(curl -L -s https://raw.githubusercontent.com/gugun09/V2Ray_ws-tls_bash_onekey/${github_branch}/v2ray.sh)
        ;;
    4)
